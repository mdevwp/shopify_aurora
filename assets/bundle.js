...
class FeaturedNavigation extends base_component_1.BaseComponent {
    ...
    constructor() {
        ...
        this.mobileVisibleImage = null;
        this.mobileVisibleButton = null;
    ...
    initMobile = () => {
        ...
        this.lines.forEach(line => {
            line.addEventListener('click', this.handleLineTouch);
        });
    };
    deInitMobile = () => {
        this.toggleImageActiveClass(this.mobileVisibleImage, 'remove');
        this.toggleButtonActiveClass(this.mobileVisibleButton, 'remove');
        this.mobileVisibleImage = null;
        this.mobileVisibleButton = null;
        this.lines.forEach(line => {
            line.removeEventListener('click', this.handleLineTouch);
        });
        window.removeEventListener('click', this.handleMobileOutsideClick);
    };
    handleLineTouch = (event) => {
        if (this.isMobile) {
            const { targetImage, targetButton } = this.getTargetImageAndButton(event);
            this.setRects(targetImage, targetButton);
            if (targetImage) {
                targetImage.style.translate = `${event.clientX - this.imageWidth}px ${event.clientY - this.imageHeight}px`;
            }
            if (targetButton) {
                targetButton.style.translate = `${event.clientX - this.buttonWidth}px ${event.clientY - this.buttonHeight}px`;
            }
            if (targetButton !== this.mobileVisibleButton) {
                if (targetImage)
                    targetImage.style.scale = '0';
                if (targetButton)
                    targetButton.style.scale = '0';
                requestAnimationFrame(() => {
                    if (targetImage)
                        targetImage.style.scale = '1';
                    if (targetButton)
                        targetButton.style.scale = '1';
                    this.toggleImageActiveClass(targetImage, 'add');
                    this.toggleButtonActiveClass(targetButton, 'add');
                    this.toggleImageActiveClass(this.mobileVisibleImage, 'remove');
                    this.toggleButtonActiveClass(this.mobileVisibleButton, 'remove');
                    this.mobileVisibleImage = targetImage;
                    this.mobileVisibleButton = targetButton;
                });
            }
            if (!this.mobileVisibleImage && !this.mobileVisibleButton) {
                window.addEventListener('click', this.handleMobileOutsideClick);
                window.addEventListener('scroll', this.handleScroll);
            }
        }
    };
    handleScroll = async () => {
        this.hideMobileVisible();
    };
    handleMobileOutsideClick = async ({ target }) => {
        if (!target.hasAttribute('data-navigation-line')) {
            this.hideMobileVisible();
        }
    };
    async hideMobileVisible() {
        if (this.mobileVisibleImage)
            this.mobileVisibleImage.style.scale = '0';
        if (this.mobileVisibleButton)
            this.mobileVisibleButton.style.scale = '0';
        await (0, utils_1.transitionToPromise)(this.mobileVisibleImage ?? this.mobileVisibleButton);
        this.toggleImageActiveClass(this.mobileVisibleImage, 'remove');
        this.toggleButtonActiveClass(this.mobileVisibleButton, 'remove');
        this.mobileVisibleImage = null;
        this.mobileVisibleButton = null;
    ...
    handleMouseMove = (event) => {
        if (!this.forceStopped) {
            ...
            const { x, y } = this.getMouseCoordinates(event.clientX, event.clientY);
            this.x = x;
            this.y = y;
            if (!this.isAnimating) {
                this.startAnimation();
...
class ImageSlider extends base_component_1.BaseComponent {
    ...
    constructor() {
        ...
        if (this.withRotateImages) {
            ...
            this.dots = (0, utils_1.$list)('[data-progress-dot]');
    ...
    mountComponent() {
        if (this.withProgressBar && !this.withRotateImages) {
            (0, utils_1.whenDefined)('carousel-component').then(() => {
                this.carousel?.on('carouselSelect', this.handleCarouselSelect);
            });
        }
        if (this.withRotateImages) {
        ...
        if (this.withRotateImages && !this.withVerticalScrollText) {
    ...
    unmountComponent() {
        if (this.withProgressBar && !this.withRotateImages) {
            this.carousel?.off('carouselSelect', this.handleCarouselSelect);
        }
        if (this.withRotateImages) {
            this.arrows?.forEach(arrow => {
    ...
    toggleArrowsDisabled = () => {
        this.arrows[0].toggleAttribute('disabled', this.activeIndex === 0);
        this.arrows[1].toggleAttribute('disabled', this.activeIndex === this.length);
    };
...
class AccordeonComponent extends base_component_1.BaseComponent {
    ...
    isExpanded;
    mountComponent() {
        if(!window.accordions) window.accordions = [];
    ...
    async toggle() {
        ...
        if(window.collapse_other_rows) {
            let currentAccordion = this;
            
            window.accordions.forEach(function(accordion, index) {
                if(currentAccordion !== accordion) accordion.hide();
            });
        }
    }
    updateHeight() {
...
class BodyElement extends base_component_1.BaseComponent {
    ...
    showOverlay(key, styles) {
        ...
        this.setScrollLock();
        this.setOverlayStyles({ ...defaultStyles, ...styles });
...
class HorizontalProductCard extends card_1.Card {
    ...
    async callQuickView(withOpen) {
        ...
        quickView.requestProductFromUrl(url, id);
        if (withOpen) {
...
class CarouselButton extends base_component_1.BaseComponent {
    ...
    updateVisible() {
        ...
        this.toggleAttribute('has-not-slides-not-in-view', slides.includes(index));
    }
    trigger() {
...
class HeaderFloatElement extends _1.FloatElement {
    ...
    handleBlockSelect = (0, debounce_1.debounce)(({ detail: { blockId, sectionId } }) => {
        if (this.dataset.sectionId === sectionId) {
            if (this.header.navContainer.hasAttribute('hidden')) {
                return;
            }
...
class MaxLinesComponent extends base_component_1.BaseComponent {
    ...
    update() {
        ...
        const rounded = Math.round(height / lineHeight);
        const floored = Math.floor(height / lineHeight);
        let maxLines = rounded - floored > 0.1 ? rounded : floored;
        if (diff > 2) {
...
class ShapeSwatch extends base_component_1.BaseComponent {
    ...
    setPropertyValue(value) {
        ...
        if (isUrl) {
            ...
            // this.element.style.setProperty('--gsc-color-swatch-value', ``)
            this.setSimilarlyBgColorStatus(false);
        }
        else if (isGradient) {
            ...
            const colorMap = {
                ...
                "malachitgrün": "#325a41",
                "maigrün": "#c3d6a7",
                "mehrfarbig": "#000000", // Assuming a default value since no specific hex was provided
                "minzgrün": "#94d9a9",
                "moosgrün": "#0b8e60",
                "nachtblau": "#0b017f",
                "nussbraun": "#b7672a",
            ...
            this.setSimilarlyBgColorStatus(isSimilarlyBgColor);
            this.element.style.setProperty('--gsc-color-swatch-value', filteredValue);
        }
    }
...
class SidebarComponent extends base_component_1.BaseComponent {
    ...
    constructor() {
        ...
        if (this.hasAttribute('is-open-on-init')) {
            ...
            const template = (0, utils_1.$el)(`[data-sidebar-template="${this.id}"]`, this);
...
class CartDrawerRelatedProductsBlock extends base_component_1.BaseComponent {
    ...
    checkHasAsideProducts() {
        ...
        if (isAsideProducts) {
            const products = (0, utils_1.$list)('[is-aside-related-products] vertical-product-card');
...
class CartDrawerRelatedProducts extends base_component_1.BaseComponent {
    ...
    loadProducts() {
        ...
        return fetch(this.dataset.url)
            ...
            .then(text => {
            this.hidePreloader();
            const html = (0, dom_1.parseHTML)(text);
            const template = (0, utils_1.$el)(CART_DRAWER_TEMPLATE_SELECTOR, html);
...
class CartDrawer extends sidebar_1.SidebarComponent {
    ...
    update() {
        this.cartAPI
            ...
            .then(responseText => {
            const html = (0, dom_1.parseHTML)(responseText);
            this.lastDrawerBody = this.getTemplateContent(html);
        })
            .catch(() => {
            this.showError('');
        })
...
class HeaderComponent extends base_component_1.BaseComponent {
    ...
    checkIsNavContainerFitAllLinks = () => {
        ...
        if (this.headerGrid.className.includes('header__grid--4') ||
            ...
            prevElementRight = navFirstItemLeft;
            nextElementleft = this.getBoundingClientRect().right - 50;
        }
        const burgerIconWidth = this.burgerIcon.getBoundingClientRect().width;
    ...
    setTransparentMargin = () => {
        ...
        if (isToSetMargin) {
            ...
            this.section.style.marginBottom = '0px';
        }
    };
        ...
        const filtered = images.filter(img => {
            ...
            return options.some((optionName, index) => {
                if (patternOption.toLowerCase() === optionName.toLowerCase()) {
                    return patternValues.some(patternValue => patternValue.toLowerCase() ===
...
class ProductDetails extends base_component_1.BaseComponent {
    ...
    scrollToActiveTab = () => {
        ...
        if (!tabComponents || !carousel) {
            return;
        }
        const activeTab = tabComponents
            .getNames()
            .findIndex(tab => tab.dataset.id === tabComponents.dataset.selectedTabId);
        carousel?.embla.scrollTo(activeTab);
    };
}
...
class RecentlyViewed extends base_component_1.BaseComponent {
    ...
    products;
    containers;
    static localStorageSelector = 'AuroraTheme-RecentlyViewed';
    static defaultLimit = 50;
    constructor() {
        super();
        this.defaultCardsLimit = RecentlyViewed.defaultLimit;
        this.defaultCardImgRatio = 'portrait';
        this.defaultCardAlignment = 'left';
        this.containerAttribute = '[recently-viewed-container]';
        this.localStorageSelector = RecentlyViewed.localStorageSelector;
        this.cardTemplateSelector = 'product-card-template';
        this.cardLimit = +this.dataset.cardLimit ?? this.defaultCardsLimit;
        this.cardImgRatio = this.dataset.imgRatio ?? this.defaultCardImgRatio;
        this.cardAlignment = this.dataset.cardAlignment ?? this.defaultCardAlignment;
        this.cardSelector = 'product-card';
        this.cardAlignmentClass = `product-card--${this.cardAlignment}-alignment`;
        this.cardInnerSelector = '[data-product-card-inner]';
        this.shapeClass = `shape--${this.cardImgRatio}`;
        this.products = [];
        this.containers = (0, utils_1.$list)(this.containerAttribute, this);
    }
    async mountComponent() {
    ...
    fetchItems = async () => {
        ...
        const currentSavedProducts = JSON.parse(localStorage.getItem(this.localStorageSelector))?.slice(0, this.cardLimit);
        ...
        try {
            for (const handle of currentSavedProducts) {
                ...
                const node = this.createNodeByString(section[this.cardTemplateSelector]);
                if (node) {
    ...
    setItems = () => {
        this.containers.forEach(container => {
            const products = this.products
                .filter(product => !!product.querySelector('product-card'))
                .map(product => product.cloneNode(true));
            container.replaceChildren(...products);
        });
    };
    setVisible(isVisible) {
        this.classList.toggle('hidden', !isVisible);
        const carousels = (0, utils_1.$list)('carousel-component', this);
        carousels.forEach(carousel => {
            const parent = carousel.parentNode;
            parent.replaceChild(carousel.cloneNode(true), carousel);
        });
    }
    createNodeByString(productString) {
        const html = new DOMParser().parseFromString(productString, 'text/html');
        const node = (0, utils_1.$el)(`.${this.cardTemplateSelector}`, html);
        (0, utils_1.$el)(this.cardSelector, node)?.classList.add(this.cardAlignmentClass);
        (0, utils_1.$el)(this.cardInnerSelector, node)?.classList.add(this.shapeClass);
        if (this.shapeClass.includes('fit')) {
            (0, utils_1.$el)(this.cardInnerSelector, node)?.classList.add('shape--fit');
        }
        return node;
    }
    static updateProductsInLocalStorage(handle) {
        ...
        const currentSavedProducts = JSON.parse(localStorage.getItem(this.localStorageSelector)) ?? [];
        const isExist = currentSavedProducts?.find((item) => item === handle);
        const newProducts = isExist
            ? currentSavedProducts
            : [handle, ...currentSavedProducts].slice(0, this.defaultLimit);
        localStorage.setItem(this.localStorageSelector, JSON.stringify(newProducts));
    }
}
...
class SearchSidebarFormWrapper extends base_component_1.BaseComponent {
    ...
    renderResults() {
        this.cache[this.searchQuery]
            ? this.renderFromCache(this.searchQuery)
    ...
    renderFromCache(queryKey) {
        try {
            this.updateResultsFromHTML(this.cache[queryKey]);
        }
        catch (error) {
            console.log('Error in search component when try update from cache');
        }
    }
    renderFromUrl(queryKey) {
        ...
        this.predictiveSearchAPI
            ...
            .then(text => {
            const html = (0, dom_1.parseHTML)(text);
            this.cache[queryKey] = html;
            this.updateResultsFromHTML(html.body);
        })
            .catch(() => {
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.replaceNodeChildren = exports.parseHTML = exports.skipTransition = exports.setStyleVariable = exports.getTemplateFirstChild = exports.getTemplateContent = exports.getStylePropValue = exports.hideElement = exports.showElement = void 0;
...
const getTargets = element => {
    const targets = (0, exports.$list)((0, exports.getFocusTargets)(), element).filter(target => target.clientWidth > 0);
    const firstTarget = targets[0];
    const lastTarget = targets[targets.length - 1];
    return { targets, firstTarget, lastTarget };
};
...
/******/ 	!function() {
/******/ 		__webpack_require__.h = function() { return "6a78149d060bd4753757"; }
/******/ 	}();
/******/ 	
